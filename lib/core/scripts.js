...
var hook = function (action, ordered, config, logger, packages, installed, json) {
	...
	var _addPromise = function (config, action, names, logger) {
		if (config.scripts && config.scripts[action]) {
			var commands = config.scripts[action];
	...
	_addPromise(config, action, packageNames, logger);
	...
	Object.keys(flattened).forEach(function (name) {
		...
		_addPromise(Config.read(pkgPath), action, packageNames, logger);
	});

	return Q.all(promises.filter(function(item) {
		return !!item;
	}));
};
